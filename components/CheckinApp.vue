...
<template>
  <div>
    <div v-if="ShowCheck" class="fixed top-0 left-0 flex justify-center items-end w-screen h-full">
        ...
        <div>
          ...
          <div class="bg-white shadow-lg">
            <div class="px-8">
              ...
              <div class="py-8">
                <button class="rounded-lg w-full bg-blue-600 py-4 font-semibold text-white text-base hover:opacity-70 transition-all focus:ring-4 focus:ring-blue-200 focus:opacity-100" @click="checkinapp">
                  Upload
                </button>
              </div>
            </div>
  ...
  methods: {
    ...
    async checkinapp () {
      try {
        ...
        await this.$axios.$post('/api/checkin', formData,
          {
            headers: {
          ...
          this.$toast.success(' Checkin berhasil')
